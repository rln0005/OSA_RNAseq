...
res2 <-  within(res_all, rank <- sign(log2FoldChange) * -log10(pvalue))
res2 
...
####  Make a file for the normalized expression DATA -- not using any of this for analysis
...
nt <- assay(nt)
colnames(nt) <- coldata[,1]
### Write the file as a tab delimited file with .txt for import into GSEA
write.csv(as.data.frame(normCounts), sep="\t", file="NormalizedCounts.csv") 
...
write.csv(as.data.frame(res_sigFC_up), file="DEGs_sigFC_up.csv", row.names=FALSE)
write.csv(as.data.frame(res_sigFC_down), file="DEGs_sigFC_down.csv", row.names=FALSE)
